...
function Tile({ id, name, ingredients, directions, time }: Recipe) {
  ...
  return (
    <div onClick={() => openModal(recipe.id)}>
      ...
      <Modal argId={recipe.id} argName={recipe.name} argIngredients={ingredients} argDirections={recipe.directions} argTime={recipe.time} isOpen={checkIsOpen} functionOnClose={closeModal}></Modal>
    </div>
  );
}
